...
public class TestArchiverTask {
	...
	public void testResolve() {
		...
		assertEquals("http://seek.org/example/abc",
				ArchiverTask.resolveToURI(base, "abc"));
		assertEquals("http://seek.org/example/abc/def",
				ArchiverTask.resolveToURI(base, "abc/def"));
		assertEquals("http://seek.org/example/abc.def",
				ArchiverTask.resolveToURI(base, "abc.def"));
		assertEquals("http://seek.org/example/abc%40def",
				ArchiverTask.resolveToURI(base, "abc@def"));
		assertEquals("http://seek.org/example/abc+def",
				ArchiverTask.resolveToURI(base, "abc def"));
		assertEquals("http://seek.org/example/abc+def+ghi/jkl.mn",
				ArchiverTask.resolveToURI(base, "abc def ghi/jkl.mn"));
		assertEquals("http://seek.org/example/abc+def%E2%82%AC.ghi",
